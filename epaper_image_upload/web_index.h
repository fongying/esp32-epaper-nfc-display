...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .battery-badge.low {
      border-color:rgba(209,15,31,.5);
      background:#fff8f8;
      color:var(--red);
    }
    .battery-badge.missing {
      background:var(--cream);
      color:var(--muted);
      font-weight:700;
    }
...
<body>
  <main class="app">
    <header class="topbar">
      ...
      <div class="top-actions">
        <div id="batteryBadge" class="battery-badge missing">電池讀取中</div>
  ...
  <script>
    ...
    const batteryBadgeEl = document.getElementById('batteryBadge');
    ...
    function batteryLabel(data) {
      if (!data || !data.present) return '電池未偵測';
      return `電池 ${data.percent}% · ${formatBatteryVoltage(data.mv)}`;
    }

    function renderBatteryBadge(data) {
      batteryBadgeEl.textContent = batteryLabel(data);
      batteryBadgeEl.classList.toggle('low', !!(data && data.low));
      batteryBadgeEl.classList.toggle('missing', !(data && data.present));
    }

    async function loadBatteryStatus() {
      if (localPreview) {
        return { mv:3910, percent:82, present:true, low:false, otaAllowed:true };
      }
    ...
    async function checkFirmwareUpdate() {
      try {
        ...
        const battery = await loadBatteryStatus().catch(err => {
          console.warn('battery check failed', err);
          return null;
        });
        if (battery) renderBatteryBadge(battery);
        ...
        updateTipEl.textContent = `發現新版本 ${manifest.version}。${manifest.notes || ''} ${battery ? batteryLabel(battery) : ''}`;
        if (battery && battery.present && !battery.otaAllowed) {
          setStatus('電量低於 20%，請充電後再更新。');
          return;
        }
        const updateMessage = [
          `發現新版本 ${manifest.version}`,
          `目前版本 ${current.version}`,
          battery ? batteryLabel(battery) : '電池狀態讀取失敗，請確認供電穩定。',
          '',
          manifest.notes || '',
          battery && !battery.present ? '未偵測到電池，請確認 USB 或外部供電穩定。' : '',
          '',
          '要更新嗎？'
        ].filter(Boolean).join('\n');
        const ok = confirm(updateMessage);
        if (ok) {
          await applyFirmwareUpdate(manifest, battery);
        } else {
          setStatus('已略過此次更新提示。');
        }
      } catch (err) {
    ...
    async function applyFirmwareUpdate(manifest, battery) {
    ...
    updateProps();
    refreshNfcStatus();
    loadBatteryStatus().then(renderBatteryBadge).catch(err => {
      batteryBadgeEl.textContent = `電池讀取失敗`;
      batteryBadgeEl.classList.add('missing');
      console.warn('battery load failed', err);
    });
    checkFirmwareUpdate();
  </script>
</body>
