...
Battery + -> 220kΩ -> GPIO2/A2 -> 220kΩ -> GND
...
- Wi-Fi transmit power is limited to `WIFI_POWER_8_5dBm` before STA/AP startup to reduce ESP32-C3 Super Mini brownout risk
...
- OTA is blocked when a detected battery is below 20%; charge the battery before updating
...
- Reads battery voltage from GPIO2/A2 through a 220kΩ/220kΩ divider
- Shows battery percentage and voltage on the designer page and `/config`
- Shows a low-battery icon at the top-right of the e-paper output when battery is below 20%
...
- GPIO2/A2 battery reading assumes a 1/2 divider. `4.2V` maps to 100%, `3.3V` maps to 0%, and readings below `0.5V` are treated as not detected.
